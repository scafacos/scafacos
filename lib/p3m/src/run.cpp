...
domain_decompose(Solver *d, fcs_gridsort_t *gridsort,
	...
	p3m_float box_a[3] = {d->box_l[0], 0.0, 0.0 };
	p3m_float box_b[3] = {0.0, d->box_l[1], 0.0 };
	p3m_float box_c[3] = {0.0, 0.0, d->box_l[2] };
	p3m_int num_particles;
...
void compute_far(Solver* d,
	...
	P3M_INFO(printf( "ip3m_p3m_run() [IK] finished.\n"));
}
#endif
...
get_ca_points(Solver *d, p3m_float real_pos[3], p3m_int shifted) {
	...
	for (p3m_int dim=0; dim<3; dim++) {
		...
		p3m_float pos = (real_pos[dim] - d->local_grid.ld_pos[dim]) * d->ai[dim];
...
assign_charges(Solver* d,
	...
	const p3m_int q2off = d->local_grid.q_2_off;
	const p3m_int q21off = d->local_grid.q_21_off;

	/* init local charge grid */
	for (p3m_int i=0; i<d->local_grid.size; i++) data[i] = 0.0;

	/* now assign the charges */
	for (p3m_int pid=0; pid < num_real_particles; pid++) {
		const p3m_float q = charges[pid];
		p3m_int linind_grid = get_ca_points(d, &positions[pid*3], shifted);

		/* Loop over all ca grid points nearby and compute charge assignment fraction */
		for (p3m_float *caf_x = d->cafx->begin(); caf_x < d->cafx->end(); caf_x++) {
			for (p3m_float *caf_y = d->cafy->begin(); caf_y < d->cafy->end(); caf_y++) {
				p3m_float caf_xy = *caf_x * *caf_y;
				for (p3m_float *caf_z = d->cafz->begin(); caf_z < d->cafz->end(); caf_z++) {
					/* add it to the grid */
...
void ik_diff(Solver* d, int dim) {
	...
	p3m_int dim_rs = (dim+d->ks_pnum)%3;
...
p3m_float compute_total_energy(Solver* d) {
	...
	p3m_float prefactor = 1.0 / (2.0 * d->box_l[0] * d->box_l[1] * d->box_l[2]);
...
assign_potentials(Solver* d,
	...
	const p3m_int q2off = d->local_grid.q_2_off;
	const p3m_int q21off = d->local_grid.q_21_off;
	const p3m_float prefactor = 1.0 / (d->box_l[0] * d->box_l[1] * d->box_l[2]);
...
assign_fields_ik(Solver* d,
	...
	const p3m_int q2off = d->local_grid.q_2_off;
	const p3m_int q21off = d->local_grid.q_21_off;
	const p3m_float prefactor = 1.0 / (2.0 * d->box_l[0] * d->box_l[1] * d->box_l[2]);
	const p3m_int dim_rs = (dim+d->ks_pnum) % 3;
...
assign_fields_ad(Solver* d,
	...
	const p3m_int q21off = d->local_grid.q_21_off;
	const p3m_float prefactor = 1.0 / (d->box_l[0] * d->box_l[1] * d->box_l[2]);
	const p3m_float l_x_inv = 1.0/d->box_l[0];
	const p3m_float l_y_inv = 1.0/d->box_l[1];
	const p3m_float l_z_inv = 1.0/d->box_l[2];
	const p3m_float grid[3] =
	{ (p3m_float)d->grid[0],
			(p3m_float)d->grid[1],
			(p3m_float)d->grid[2] };
	...
	for (p3m_int pid = 0; pid < num_real_particles; pid++) {
		...
		p3m_float *caf_x_d = d->cafx_d->begin();
		for (p3m_float *caf_x = d->cafx->begin(); caf_x < d->cafx->end(); caf_x++) {
			p3m_float *caf_y_d = d->cafy_d->begin();
			for (p3m_float *caf_y = d->cafy->begin(); caf_y < d->cafy->end(); caf_y++) {
				p3m_float *caf_z_d = d->cafz_d->begin();
				for (p3m_float *caf_z = d->cafz->begin(); caf_z < d->cafz->end(); caf_z++) {
