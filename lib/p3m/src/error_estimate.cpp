...
namespace P3M {
  ...
  static p3m_float 
  k_space_error_sum1
  (p3m_int n, p3m_float grid_i, p3m_int cao);

  static void 
  k_space_error_sum2
  (p3m_int nx, p3m_int ny, p3m_int nz, 
   p3m_int grid[3], p3m_float grid_i[3], 
   p3m_int cao, p3m_float alpha_L_i, 
  ...
  */
  ...
  real_space_error(data_struct *d) {
    d->rs_error = (2.0*d->sum_q2*exp(-SQR(d->r_cut*d->alpha))) 
      / (sqrt((p3m_float)d->sum_qpart*d->r_cut*d->box_l[0]*d->box_l[1]*d->box_l[2]));
  }
  ...
  */
  ...
  k_space_error(data_struct *d) {
    ...
    for (p3m_int ix = min_ix; ix < max_ix; ix++) {
      ...
      p3m_int ny = ix % (d->grid[1]*d->grid[2]) / d->grid[2] - d->grid[1]/2;
      ...
      if (nx != 0 || ny != 0 || nz != 0) {
        ...
        p3m_float cs = ctan_x * ctan_y * k_space_error_sum1(nz,grid_i[2],d->cao);
...
#endif
#else
    p3m_float ks_error = 2.0*d->sum_q2*sqrt
      ( he_q / (p3m_float)d->sum_qpart / (d->box_l[0]*d->box_l[1]*d->box_l[2]) );
#endif
  ...
  void 
  k_space_error_sum2(p3m_int nx, p3m_int ny, p3m_int nz, 
                     p3m_int grid[3], p3m_float grid_i[3], 
                     p3m_int cao, p3m_float alpha_L_i, 
                     p3m_float *alias1, p3m_float *alias2)
  {
    ...
    for (p3m_int mx=-P3M_BRILLOUIN; mx<=P3M_BRILLOUIN; mx++) {
      ...
      for (p3m_int my=-P3M_BRILLOUIN; my<=P3M_BRILLOUIN; my++) {
        ...
        for (p3m_int mz=-P3M_BRILLOUIN; mz<=P3M_BRILLOUIN; mz++) {
          ...
          p3m_float U2 = pow(sinc(fnmx)*sinc(fnmy)*sinc(fnmz), 2.0*cao);
  ...
  /** aliasing sum used by \ref k_space_error. */
  static void 
  k_space_error_sum2_adi(p3m_int nx, p3m_int ny, p3m_int nz, 
                         p3m_int grid[3], p3m_float grid_i[3], 
                         p3m_int cao, p3m_float alpha_L_i, 
                         p3m_float *alias1, p3m_float *alias2,
                         p3m_float *alias3, p3m_float *alias4,
                         p3m_float *alias5, p3m_float *alias6)
  {
    p3m_float prefactor = SQR(M_PI*alpha_L_i);
