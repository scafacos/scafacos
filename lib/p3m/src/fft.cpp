...
namespace P3M {
  ...
  /***************************************************/
  static int 
  find_comm_groups(comm_struct *comm,
                   int grid1[3], int grid2[3], 
                   int *node_list1, int *node_list2,
                   int *group, int *pos, int *my_pos);
  static int 
  calc_local_grid(int n_pos[3], int n_grid[3], 
                  int grid[3], p3m_float grid_off[3],
                  int loc_grid[3], int start[3]);
  static int 
  calc_send_block(int pos1[3], int grid1[3], 
                  int pos2[3], int grid2[3],
                  int grid[3], 
                  p3m_float grid_off[3], int block[6]);
  static void 
  pack_block_permute1(p3m_float *in, p3m_float *out, 
                      int start[3], int size[3],
                      int dim[3], int element);
  
  static void 
  pack_block_permute2(p3m_float *in, p3m_float *out, 
                      int start[3], int size[3],
                      int dim[3],int element);
  
  static void 
  print_global_grid(comm_struct *comm,
                    Parallel3DFFT::forward_plan plan, 
                    p3m_float *data, p3m_int element, 
                    p3m_int num);
  ...
  void Parallel3DFFT::forward_plan::print() {
    ...
    printf("        dir=%d, row_dir=%d, n_permute=%d, n_ffts=%d\n",
           dir,  row_dir, n_permute, n_ffts);

    printf("        local: old_grid=(%d,%d,%d), new_grid=(%d,%d,%d), start=(%d,%d,%d)\n",
           old_grid[0],  old_grid[1],  old_grid[2], 
           new_grid[0],  new_grid[1],  new_grid[2], 
           start[0], start[1],  start[2]);
    printf("        new_size %d\n", new_size);

    printf("        g_size=%d group=(",g_size);
    for (int i=0; i<g_size-1; i++) printf("%d,", group[i]);
    printf("%d)\n",group[g_size-1]);
    
    printf("        send=[");
    for(int i=0; i<g_size; i++) 
      printf("(%d,%d,%d)+(%d,%d,%d), ",
             send_block[6*i+0], send_block[6*i+1], send_block[6*i+2],
             send_block[6*i+3], send_block[6*i+4], send_block[6*i+5]);
    printf("]\n");
    printf("        recv=[");
    for(int i=0;i<g_size;i++) 
      printf("(%d,%d,%d)+(%d,%d,%d), ",
             recv_block[6*i+0], recv_block[6*i+1], recv_block[6*i+2],
             recv_block[6*i+3], recv_block[6*i+4], recv_block[6*i+5]);
    printf("]\n");
  }
  ...
  p3m_float* Parallel3DFFT::malloc_data() {
    return static_cast<p3m_float*>(fftw_malloc(max_grid_size*sizeof(p3m_float)));
  }
  ...
  prepare(p3m_int *local_grid_dim, p3m_int *local_grid_margin, 
    ...
    if(g3d[2] == 1) { 
      ...
      plan[1].row_dir=-1;
      if (g2d[0]%g3d[0] == 0) {
        if (g2d[1]%g3d[1] == 0) plan[1].row_dir=2;
    ...
    for (int i=1; i<4; i++) {
      plan[i].g_size =
        find_comm_groups(comm, 
                         n_grid[i-1], n_grid[i], n_id[i-1], n_id[i], 
                         plan[i].group, n_pos[i], my_pos[i]);
      if (plan[i].g_size==-1) {
        /* try permutation */
      ...
      plan[i].send_block = 
        static_cast<p3m_int*>(realloc(plan[i].send_block, 
                                      6*plan[i].g_size*sizeof(p3m_int)));
      plan[i].send_size = 
        static_cast<p3m_int*>(realloc(plan[i].send_size, 
                                      1*plan[i].g_size*sizeof(p3m_int)));
      plan[i].recv_block = 
        static_cast<p3m_int*>(realloc(plan[i].recv_block, 
                                      6*plan[i].g_size*sizeof(p3m_int)));
      plan[i].recv_size = 
        static_cast<p3m_int*>(realloc(plan[i].recv_size, 
                                      1*plan[i].g_size*sizeof(p3m_int)));
      
      plan[i].new_size = calc_local_grid(my_pos[i], n_grid[i], global_grid_dim,
                                         global_grid_off, plan[i].new_grid, 
                                         plan[i].start);  
      ...
      for (int j = 0; j<plan[i].g_size; j++) {
        ...
        plan[i].send_size[j] 
          = calc_send_block(my_pos[i-1], n_grid[i-1], &(n_pos[i][3*node]), n_grid[i],
                                global_grid_dim, global_grid_off, 
                                &(plan[i].send_block[6*j]));
        permute_ifield(&(plan[i].send_block[6*j]),3,-(plan[i-1].n_permute));
        permute_ifield(&(plan[i].send_block[6*j+3]),3,-(plan[i-1].n_permute));
        ...
        plan[i].recv_size[j] 
          = calc_send_block(my_pos[i], n_grid[i], &(n_pos[i-1][3*node]), n_grid[i-1],
                            global_grid_dim,global_grid_off,&(plan[i].recv_block[6*j]));
        permute_ifield(&(plan[i].recv_block[6*j]),3,-(plan[i].n_permute));
        permute_ifield(&(plan[i].recv_block[6*j+3]),3,-(plan[i].n_permute));
    ...
    P3M_DEBUG(printf("      max_comm_size = %d, max_grid_size = %d\n",
    ...
    send_buf = (p3m_float *)realloc(send_buf, max_comm_size*sizeof(p3m_float));
    recv_buf = (p3m_float *)realloc(recv_buf, max_comm_size*sizeof(p3m_float));

    if (data_buf != NULL) free_data(data_buf);
    ...
    fftw_complex *c_data_buf = 
      reinterpret_cast<fftw_complex*>(data_buf);
    ...
    for (int i=1; i<4; i++) {
      ...
      sprintf(wisdom_file_name,"fftw3_1d_wisdom_forw_n%d.file", plan[i].new_grid[2]);
      ...
      if (is_prepared) 
        fftw_destroy_plan(plan[i].fftw_plan);
      //printf("plan[%d].n_ffts=%d\n",i,plan[i].n_ffts);
      plan[i].fftw_plan =
        fftw_plan_many_dft(1,&plan[i].new_grid[2],plan[i].n_ffts,
                           c_data_buf,NULL,1,plan[i].new_grid[2],
                           c_data_buf,NULL,1,plan[i].new_grid[2],
                           plan[i].dir,FFTW_PATIENT);
    ...
    for (int i=1; i<4; i++) {
      ...
      if (is_prepared) 
        fftw_destroy_plan(back[i].fftw_plan);
      back[i].fftw_plan =
        fftw_plan_many_dft(1,&plan[i].new_grid[2],plan[i].n_ffts,
                           c_data_buf, NULL, 1, plan[i].new_grid[2],
                           c_data_buf, NULL, 1, plan[i].new_grid[2],
  ...
  void Parallel3DFFT::forward(p3m_float *data) {
    ...
    */
    ...
    fftw_execute_dft(plan[1].fftw_plan,c_data,c_data);
    ...
    fftw_execute_dft(plan[2].fftw_plan,c_data_buf,c_data_buf);
    /* ===== third direction  ===== */
    ...
    /* perform FFT (in/out is data)*/
    fftw_execute_dft(plan[3].fftw_plan,c_data,c_data);
  ...
  void Parallel3DFFT::backward(p3m_float *data)
  {
    ...
    fftw_execute_dft(back[3].fftw_plan,c_data,c_data);
    ...
    fftw_execute_dft(back[2].fftw_plan, c_data_buf, c_data_buf);
    ...
    /* perform FFT (in is data) */
    fftw_execute_dft(back[1].fftw_plan,c_data,c_data);
#ifndef P3M_INTERLACE
   ...
   */
  void Parallel3DFFT::
  forward_grid_comm(forward_plan plan, p3m_float *in, p3m_float *out) {
    for(int i=0; i<plan.g_size; i++) {   
      ...
      if (plan.group[i]<comm->rank) {       /* send first, receive second */
        MPI_Send(send_buf, plan.send_size[i], P3M_MPI_FLOAT, 
                 plan.group[i], REQ_FFT_FORW, comm->mpicomm);
        MPI_Recv(recv_buf, plan.recv_size[i], P3M_MPI_FLOAT, 
                 plan.group[i], REQ_FFT_FORW, comm->mpicomm, MPI_STATUS_IGNORE); 	
      }
   ...
   */
  void Parallel3DFFT::
  backward_grid_comm(forward_plan plan_f, backward_plan plan_b, 
    ...
    for (int i=0; i<plan_f.g_size; i++) {
      ...
      if (plan_f.group[i]<comm->rank) {       /* send first, receive second */
        ...
        MPI_Recv(recv_buf, plan_f.send_size[i], P3M_MPI_FLOAT, 
                 plan_f.group[i], REQ_FFT_BACK, comm->mpicomm, MPI_STATUS_IGNORE); 	
      }
   ...
   */
  p3m_int 
  calc_local_grid(p3m_int n_pos[3], p3m_int n_grid[3], p3m_int grid[3], 
   ...
   */
  p3m_int 
  calc_send_block(p3m_int pos1[3], p3m_int node_grid1[3], 
   ...
   */
  void Parallel3DFFT::
  pack_block(p3m_float *in, p3m_float *out, p3m_int start[3], 
   ...
   *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
   */
  static void pack_block_permute1(p3m_float *in, p3m_float *out, 
   ...
   *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
   */
  static void pack_block_permute2(p3m_float *in, p3m_float *out, 
                                  p3m_int start[3], p3m_int size[3], 
                                  p3m_int dim[3],p3m_int element)
  {
    /* slow,mid and fast changing indices for input  grid */
      ...
      for(int i0 = grid-1; i0 >= 0; i0--) {
        for(int i1 = start1; i1 < imin(start1+block1,grid); i1++) {
          for(int i2 = 0; i2 < grid; i2++) {
            my = (i0>=st[0] && i0<en[0] && i1>=st[1] && 
                  i1<en[1] && i2>=st[2] && i2<en[2]);

            MPI_Barrier(comm->mpicomm);
            if (my) {
              p3m_float tmp = 
                data[num+(element*((i2-st[2])+si[2]*((i1-st[1])+si[1]*(i0-st[0]))))];
              if (fabs(tmp)>1.0e-15) {
