...
namespace ScaFaCoS {
  namespace P3M {
    ...
    static void 
    fft_pack_block_permute1(fcs_float *in, fcs_float *out, 
                                 int start[3], int size[3],
                                 int dim[3], int element);

    static void 
    fft_pack_block_permute2(fcs_float *in, fcs_float *out, 
                                 int start[3], int size[3],
                                 int dim[3],int element);
    ...
    void fft_destroy(fft_data_struct *fft, fcs_float *data, fcs_float *ks_data) {
      for(int i=0;i<4;i++) {
    ...
    void fft_forw_grid_comm(fft_data_struct *fft, comm_struct *comm,
        forw_plan plan, fcs_float *in, fcs_float *out)
    {
      fcs_int i;
      MPI_Status status;
      fcs_float *tmp_ptr;
    ...
    void fft_back_grid_comm(fft_data_struct *fft,  comm_struct *comm,
                                 forw_plan plan_f,  back_plan plan_b, fcs_float *in, fcs_float *out)
    {
      fcs_int i;
      MPI_Status status;
      fcs_float *tmp_ptr;
     ...
     */
    fcs_int 
    fft_calc_local_grid(fcs_int n_pos[3], fcs_int n_grid[3], fcs_int grid[3], fcs_float grid_off[3], 
     ...
     */
    fcs_int 
    fft_calc_send_block(fcs_int pos1[3], fcs_int node_grid1[3], 
                             fcs_int pos2[3], fcs_int node_grid2[3], 
     ...
     */
    void fft_pack_block(fcs_float *in, fcs_float *out, fcs_int start[3], 
                             fcs_int size[3], fcs_int dim[3], fcs_int element)
    {
     ...
     *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
     */
    void fft_pack_block_permute1(fcs_float *in, fcs_float *out, 
     ...
     *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
     */
    void fft_pack_block_permute2(fcs_float *in, fcs_float *out, 
                                      fcs_int start[3], fcs_int size[3], 
                                      fcs_int dim[3],fcs_int element)
    {
      /* slow,mid and fast changing indices for input  grid */
    ...
    void fft_unpack_block(fcs_float *in, fcs_float *out, 
                               fcs_int start[3], fcs_int size[3], 
                               fcs_int dim[3], fcs_int element)
    {
      /* mid and slow changing indices */
