...
/***************************************************/
static void 
ifcs_fft_forw_grid_comm(ifcs_fft_data_struct *fft, 
			ifcs_p3m_comm_struct *comm,
			ifcs_fft_forw_plan plan, 
			fcs_float *in, fcs_float *out);
static void 
ifcs_fft_back_grid_comm(ifcs_fft_data_struct *fft, 
			ifcs_p3m_comm_struct *comm,
			ifcs_fft_forw_plan plan_f, 
			ifcs_fft_back_plan plan_b, 
			fcs_float *in, fcs_float *out);
static int 
ifcs_fft_find_comm_groups(ifcs_p3m_comm_struct *comm,
			  int grid1[3], int grid2[3], 
			  int *node_list1, int *node_list2,
			  int *group, int *pos, int *my_pos);
static int 
ifcs_fft_calc_local_grid(int n_pos[3], int n_grid[3], 
			 int grid[3], fcs_float grid_off[3],
			 int loc_grid[3], int start[3]);
static int 
ifcs_fft_calc_send_block(int pos1[3], int grid1[3], 
			 int pos2[3], int grid2[3],
			 int grid[3], 
			 fcs_float grid_off[3], int block[6]);
void 
ifcs_fft_pack_block(fcs_float *in, fcs_float *out, 
		    int start[3], int size[3],
		    int dim[3], int element);
static void 
ifcs_fft_pack_block_permute1(fcs_float *in, fcs_float *out, 
			     int start[3], int size[3],
			     int dim[3], int element);

static void 
ifcs_fft_pack_block_permute2(fcs_float *in, fcs_float *out, 
			     int start[3], int size[3],
			     int dim[3],int element);

#if FCS_ENABLE_DEBUG
static void 
ifcs_fft_print_global_fft_grid(ifcs_fft_data_struct *fft, 
			      ifcs_p3m_comm_struct *comm,
			      ifcs_fft_forw_plan plan, 
			      fcs_float *data, int element, int num);

static void 
ifcs_fft_print_fft_plan(ifcs_fft_forw_plan pl);
#endif
...
void ifcs_fft_destroy(ifcs_fft_data_struct *fft, fcs_float *data, fcs_float *ks_data) {
  for(int i=0;i<4;i++) {
...
ifcs_fft_prepare(ifcs_fft_data_struct *fft, ifcs_p3m_comm_struct *comm,
		 ...
		 int* global_grid_dim, fcs_float *global_grid_off,
		 int *ks_pnum) {
  int i,j;
  /* helpers */
  int mult[3];
  ...
  P3M_DEBUG(printf("    ifcs_fft_prepare() started...\n"));
  ...
  i = (fcs_int)sqrt((fcs_float)comm->size);
  while (i>=1) {
    if(comm->size % i == 0) { 
  ...
  for(i=1; i<4;i++) {
    fft->plan[i].g_size=ifcs_fft_find_comm_groups(comm, 
    ...
    if(fft->plan[i].g_size==-1) {
      ...
      fft->plan[i].g_size=ifcs_fft_find_comm_groups(comm,
					       ...
					       fft->plan[i].group, n_pos[i], my_pos[i]);
      if(fft->plan[i].g_size==-1) {
	printf("%d: INTERNAL ERROR: ifcs_fft_find_comm_groups error\n", comm->rank);
	errexit();
      }
    }

    fft->plan[i].send_block = (fcs_int *)realloc(fft->plan[i].send_block, 6*fft->plan[i].g_size*sizeof(fcs_int));
    fft->plan[i].send_size  = (fcs_int *)realloc(fft->plan[i].send_size, 1*fft->plan[i].g_size*sizeof(fcs_int));
    fft->plan[i].recv_block = (fcs_int *)realloc(fft->plan[i].recv_block, 6*fft->plan[i].g_size*sizeof(fcs_int));
    fft->plan[i].recv_size  = (fcs_int *)realloc(fft->plan[i].recv_size, 1*fft->plan[i].g_size*sizeof(fcs_int));

    fft->plan[i].new_size = ifcs_fft_calc_local_grid(my_pos[i], n_grid[i], global_grid_dim,
	...
	= ifcs_fft_calc_send_block(my_pos[i-1], n_grid[i-1], &(n_pos[i][3*node]), n_grid[i],
	...
	= ifcs_fft_calc_send_block(my_pos[i], n_grid[i], &(n_pos[i-1][3*node]), n_grid[i-1],
    ...
    /* DEBUG */
    P3M_DEBUG(ifcs_fft_print_fft_plan(fft->plan[i]));
  }
  ...
  /* === pack function === */
  for(i=1;i<4;i++) {
    fft->plan[i].pack_function = ifcs_fft_pack_block_permute2; 
    P3M_DEBUG(printf("      forw plan[%d] permute 2 \n",i));
  }
  (*ks_pnum)=6;
  if(fft->plan[1].row_dir==2) {
    fft->plan[1].pack_function = ifcs_fft_pack_block;
    P3M_DEBUG(printf("      forw plan[%d] permute 0 \n",1));
    (*ks_pnum)=4;
  }
  else if(fft->plan[1].row_dir==1) {
    fft->plan[1].pack_function = ifcs_fft_pack_block_permute1;
...
//printf("fft->plan[%d].n_ffts=%d\n",i,fft->plan[i].n_ffts);
  ...
  for (i=1; i<4; i++) {
    ...
    fft->back[i].pack_function = ifcs_fft_pack_block_permute1;
    P3M_DEBUG(printf("      back plan[%d] permute 1 \n",i));
  }
  if (fft->plan[1].row_dir==2) {
    fft->back[1].pack_function = ifcs_fft_pack_block;
    P3M_DEBUG(printf("      back plan[%d] permute 0 \n",1));
  }
  else if (fft->plan[1].row_dir==1) {
    fft->back[1].pack_function = ifcs_fft_pack_block_permute2;
    P3M_DEBUG(printf("      back plan[%d] permute 2 \n",1));
  }
  fft->init_tag=1;
  for(i=0;i<4;i++) { free(n_id[i]); free(n_pos[i]); }

  P3M_DEBUG(printf("    ifcs_fft_prepare() finished.\n"));

  return fft->max_grid_size; 
}

void ifcs_fft_perform_forw(ifcs_fft_data_struct *fft,  ifcs_p3m_comm_struct *comm,
			  fcs_float *data)
{
  fcs_int i;
  ...
  /* int m,n,o; */
  /* ===== first direction  ===== */
  P3M_DEBUG(printf("    %d: ifcs_fft_perform_forw: dir 1:\n",comm->rank));

  fftw_complex *c_data     = (fftw_complex *) data;
  fftw_complex *c_data_buf = (fftw_complex *) fft->data_buf;

  /* communication to current dir row format (in is data) */
  ifcs_fft_forw_grid_comm(fft, comm, fft->plan[1], data, fft->data_buf);
  ...
  */
  ...
  /* ===== second direction ===== */
  P3M_DEBUG_LOCAL(printf("    %d: ifcs_fft_perform_forw: dir 2\n",comm->rank));
  /* communication to current dir row format (in is data) */
  ifcs_fft_forw_grid_comm(fft, comm, fft->plan[2], data, fft->data_buf);
  /* perform FFT (in/out is fft->data_buf)*/
  fftw_execute_dft(fft->plan[2].fftw_plan,c_data_buf,c_data_buf);
  /* ===== third direction  ===== */
  P3M_DEBUG_LOCAL(printf("    %d: ifcs_fft_perform_forw: dir 3\n",comm->rank));
  /* communication to current dir row format (in is fft->data_buf) */
  ifcs_fft_forw_grid_comm(fft, comm, fft->plan[3], fft->data_buf, data);
  /* perform FFT (in/out is data)*/
  fftw_execute_dft(fft->plan[3].fftw_plan,c_data,c_data);
  /*P3M_DEBUG_LOCAL(ifcs_fft_print_global_fft_grid(fft, comm, fft->plan[3], data, 1, 0));*/

  /* REMARK: Result has to be in data. */
}

void ifcs_fft_perform_back(ifcs_fft_data_struct *fft, ifcs_p3m_comm_struct *comm,
		      fcs_float *data)
{
  fcs_int i;
  ...
  /* ===== third direction  ===== */
  P3M_DEBUG(printf("    %d: ifcs_fft_perform_back: dir 3\n",comm->rank));


  /* perform FFT (in is data) */
  fftw_execute_dft(fft->back[3].fftw_plan,c_data,c_data);
  /* communicate (in is data)*/
  ifcs_fft_back_grid_comm(fft, comm, fft->plan[3],fft->back[3],data,fft->data_buf);
 
  /* ===== second direction ===== */
  P3M_DEBUG_LOCAL(printf("    %d: ifcs_fft_perform_back: dir 2\n",comm->rank));
  /* perform FFT (in is fft->data_buf) */
  fftw_execute_dft(fft->back[2].fftw_plan,c_data_buf,c_data_buf);
  /* communicate (in is fft->data_buf) */
  ifcs_fft_back_grid_comm(fft, comm, fft->plan[2],fft->back[2],fft->data_buf,data);

  /* ===== first direction  ===== */
  P3M_DEBUG_LOCAL(printf("    %d: ifcs_fft_perform_back: dir 1\n",comm->rank));
  /* perform FFT (in is data) */
  ...
  for(i=0;i<fft->plan[1].new_size;i++) {
    ...
    if (data[2*i+1]>1e-5) {
      printf(
	     "    %d: Complex value is not zero (i=%d,data=%g)!!!\n",
  ...
  /* communicate (in is fft->data_buf) */
  ifcs_fft_back_grid_comm(fft, comm, fft->plan[1],fft->back[1],fft->data_buf,data);

  /* REMARK: Result has to be in data. */
}

/** communicate the grid data according to the given ifcs_fft_forw_plan. 
 * \param plan communication plan (see \ref ifcs_fft_forw_plan).
 * \param in   input grid.
 * \param out  output grid.
*/
void ifcs_fft_forw_grid_comm(ifcs_fft_data_struct *fft, ifcs_p3m_comm_struct *comm,
			ifcs_fft_forw_plan plan, fcs_float *in, fcs_float *out)
{
  fcs_int i;
  MPI_Status status;
  fcs_float *tmp_ptr;
  ...
  for(i=0;i<plan.g_size;i++) {   
    ...
    if (plan.group[i]<comm->rank) {       /* send first, receive second */
      MPI_Send(fft->send_buf, plan.send_size[i], FCS_MPI_FLOAT, 
	       plan.group[i], REQ_FFT_FORW, comm->mpicomm);
      MPI_Recv(fft->recv_buf, plan.recv_size[i], FCS_MPI_FLOAT, 
    ...
    ifcs_fft_unpack_block(fft->recv_buf, out, &(plan.recv_block[6*i]), 
		     &(plan.recv_block[6*i+3]), plan.new_grid, plan.element);
  }
}

/** communicate the grid data according to the given ifcs_fft_forw_plan/ifcs_fft_bakc_plan. 
 * \param plan_f communication plan (see \ref ifcs_fft_forw_plan).
 * \param plan_b additional back plan (see \ref fft.back_plan).
 * \param in     input grid.
 * \param out    output grid.
*/
void ifcs_fft_back_grid_comm(ifcs_fft_data_struct *fft,  ifcs_p3m_comm_struct *comm,
			ifcs_fft_forw_plan plan_f,  ifcs_fft_back_plan plan_b, fcs_float *in, fcs_float *out)
{
  fcs_int i;
  MPI_Status status;
  fcs_float *tmp_ptr;
  ...
  for (i=0; i<plan_f.g_size; i++) {
    ...
    if (plan_f.group[i]<comm->rank) {       /* send first, receive second */
      MPI_Send(fft->send_buf, plan_f.recv_size[i], FCS_MPI_FLOAT, 
	       plan_f.group[i], REQ_FFT_BACK, comm->mpicomm);
      MPI_Recv(fft->recv_buf, plan_f.send_size[i], FCS_MPI_FLOAT, 
    ...
    ifcs_fft_unpack_block(fft->recv_buf, out, &(plan_f.send_block[6*i]), 
...
fcs_int 
ifcs_fft_find_comm_groups(ifcs_p3m_comm_struct *comm,
			  fcs_int grid1[3], fcs_int grid2[3], 
			  fcs_int *node_list1, fcs_int *node_list2, 
			  fcs_int *group, fcs_int *pos, fcs_int *my_pos)
{
  fcs_int i;
  ...
  P3M_DEBUG(printf(						\
		    "     ifcs_fft_find_comm_groups(): for grid1=(%d,%d,%d) and grids=(%d,%d,%d)\n", \
...
*/
fcs_int 
ifcs_fft_calc_local_grid(fcs_int n_pos[3], fcs_int n_grid[3], fcs_int grid[3], fcs_float grid_off[3], 
		    fcs_int loc_grid[3], fcs_int start[3])
{
  fcs_int i, last[3], size=1;
  
  for(i=0;i<3;i++) {
    start[i] = (fcs_int)ceil( (grid[i]/(fcs_float)n_grid[i])*n_pos[i]     - grid_off[i] );
    last[i]  = (fcs_int)floor((grid[i]/(fcs_float)n_grid[i])*(n_pos[i]+1) - grid_off[i] );
    /* correct round-off errors */
    if ( (grid[i]/(fcs_float)n_grid[i])*(n_pos[i]+1) - grid_off[i] - last[i] < 1.0e-15 ) last[i]--;
    if (1.0+ (grid[i]/(fcs_float)n_grid[i])*n_pos[i]-grid_off[i]-start[i] < 1.0e-15 ) start[i]--;
...
*/
fcs_int 
ifcs_fft_calc_send_block(fcs_int pos1[3], fcs_int node_grid1[3], fcs_int pos2[3], fcs_int node_grid2[3], 
		    fcs_int grid[3], fcs_float grid_off[3], fcs_int block[6])
{
  fcs_int i,size=1;
  fcs_int grid1[3], first1[3], last1[3];
  fcs_int grid2[3], first2[3], last2[3];

  ifcs_fft_calc_local_grid(pos1, node_grid1, grid, grid_off, grid1, first1);
  ifcs_fft_calc_local_grid(pos2, node_grid2, grid, grid_off, grid2, first2);
 ...
 */
void ifcs_fft_pack_block(fcs_float *in, fcs_float *out, fcs_int start[3], fcs_int size[3], fcs_int dim[3], fcs_int element)
{
  /* mid and slow changing indices */
  fcs_int m,s;
  /* linear index of in grid, linear index of out grid */
  fcs_int li_in,li_out=0;
  /* copy size */
  fcs_int copy_size;
  /* offsets for indizes in input grid */
  fcs_int m_in_offset,s_in_offset;
  /* offsets for indizes in output grid */
  fcs_int m_out_offset;

  copy_size    = element * size[2] * sizeof(fcs_float);
 ...
 *
 * For index definition see \ref ifcs_fft_pack_block.
 *
 *  \param in      pointer to input 3d-grid.
 *  \param out     pointer to output 3d-grid (block).
 *  \param start   start index of the block in the in-grid.
 ...
 *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
 */
void ifcs_fft_pack_block_permute1(fcs_float *in, fcs_float *out, fcs_int start[3], fcs_int size[3], 
 ...
 *  \param element size of a grid element (e.g. 1 for Real, 2 for Complex).
 */
void ifcs_fft_pack_block_permute2(fcs_float *in, fcs_float *out, fcs_int start[3], fcs_int size[3], 
			     fcs_int dim[3],fcs_int element)
{
  /* slow,mid and fast changing indices for input  grid */
  fcs_int s,m,f,e;
...
void ifcs_fft_unpack_block(fcs_float *in, fcs_float *out, fcs_int start[3], fcs_int size[3], 
		      fcs_int dim[3], fcs_int element)
{
  /* mid and slow changing indices */
  fcs_int m,s;
  /* linear index of in grid, linear index of out grid */
  fcs_int li_in=0,li_out;
  /* copy size */
  fcs_int copy_size;
  /* offset for indizes in input grid */
  fcs_int m_in_offset;
  /* offsets for indizes in output grid */
  fcs_int m_out_offset,s_out_offset;
 ...
 * \param plan     fft/communication plan (see \ref ifcs_fft_forw_plan).
 * \param data     grid data.
 * \param element  element size.
 * \param num      element index to print.
*/
static void ifcs_fft_print_fft_plan(ifcs_fft_forw_plan pl)
{
  fcs_int i;

  printf("      ifcs_fft_forw_plan:\n");
...
/** Debug function to print ifcs_fft_forw_plan structure.
 * \param pl fft/communication plan (see \ref ifcs_fft_forw_plan).
 */
void ifcs_fft_print_global_fft_grid(ifcs_fft_data_struct *fft, ifcs_p3m_comm_struct *comm,
			       ifcs_fft_forw_plan plan, fcs_float *data, fcs_int element, fcs_int num)
{
  fcs_int i0,i1,i2,b=1;
  fcs_int grid,divide=0,block1=-1,start1;
  fcs_int st[3],en[3],si[3];
  fcs_int my=-1;
  fcs_float tmp;
  ...
  while(divide==0) {
    if (b*grid > 7) {
      block1=b;
      divide = (fcs_int)ceil(grid/(fcs_float)block1);
    }
    b++;
  }
  ...
  for(b=0;b<divide;b++) {
	...
	for(i2=0; i2<grid;i2++) {
	  ...
	  if (my==1) {
	    ...
	    if (fabs(tmp)>1.0e-15) {
	      if (tmp<0) printf("    %1.2" FCS_LMOD_FLOAT "e",tmp);
	      else      printf("    %1.2" FCS_LMOD_FLOAT "e",tmp);
	    }
	    else {
	      printf(" %1.2e",0.0);
	    }
