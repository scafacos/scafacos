...
/*
      ...
      start = 0; end = elems[1]; 
      for(i=1; i<nproc; i++) {
        for(j=start; j<end; j++) ARMCI_NbPutValueDouble(dsrc[me][j], &ddst_put[i][me*elems[1]+j], i, &hdl_put[j]);
      }
...
*/
      ...
      for(i=1; i<nproc; i++) {
        ...
        if((rc=ARMCI_NbPutV(&darr, 1, i, &hdl_put[i]))) ARMCI_Error("armci_nbputv failed\n",rc);
      }
      ...
      for(i=1; i<nproc; i++) {
        for(j=start; j<end; j++) {  
          if((rc=ARMCI_NbPut(&dsrc[me][j], &ddst_put[i][me*elems[1]+j], bytes, i, &hdl_put[j]))){
      ...
      for(i=1; i<nproc; i++) {
        for(j=start; j<end; j++) {  
          if((rc=ARMCI_NbPut(&dsrc[me][j], &ddst_put[i][me*elems[1]+j], bytes, i,  &aggr_hdl_put[i])))
            ARMCI_Error("armci_nbput failed\n",rc);
        }
      }
