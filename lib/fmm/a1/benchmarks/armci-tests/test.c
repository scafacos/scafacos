...
void test_aggregate() {
    ...
    /*
      ...
      for(j=start; j<end; j++) {  
        bytes = sizeof(double);
        ARMCI_NbPutValueDouble(dsrc[me][j], &ddst_put[i][me*elems[1]+j], i, &usr_hdl_put[i]);
      }
     */
      ...
      if((rc=ARMCI_NbPutV(&darr, 1, i, &usr_hdl_put[i]))) ARMCI_Error("armci_nbputv failed\n",rc);

      start = end; end = start + COUNT*NUMAGG;
      for(j=start; j<end; j+=NUMAGG) {  
        bytes = sizeof(double)*NUMAGG;
        if((rc=ARMCI_NbPutS(&dsrc[me][j], NULL, &ddst_put[i][me*elems[1]+j], NULL, &bytes, 0, i, &usr_hdl_put[i])))
      ...
      for(j=start; j<end; j+=NUMAGG) {  
        bytes = sizeof(double)*NUMAGG;
        if((rc=ARMCI_NbPut(&dsrc[me][j], &ddst_put[i][me*elems[1]+j], bytes, i, &usr_hdl_put[i])))
...
void test_implicit() {
    ...
    /*
      ...
      for(j=start; j<end; j++) {  
        bytes = sizeof(double);
        ARMCI_NbPutValueDouble(dsrc[me][j], &ddst_put[i][me*elems[1]+j], i, NULL);
      }
     */
