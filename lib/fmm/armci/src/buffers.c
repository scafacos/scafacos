...
int smallbuf_size = sizeof(buf_smext_t)*(MAX_SMALL_BUFS);
     ...
     if(DEBUG2_){
	printf("%d:armci_init_bufs: pointer %p, before align ptr=%p bufptr=%p end of region is %p  size=%zd extra=%d\n",
               armci_me,_armci_buffers,tmp,_armci_buffers->buffer,(_armci_buffers+MAX_BUFS),
               MAX_BUFS*sizeof(buf_ext_t),extra); /* remove warnings (M. Hofmann) */
	fflush(stdout);
     }
...
# ifdef BUF_EXTRA_FIELD_T
    ...
    if(DEBUG_ || 0) {
      printf("%d:buf_get:size=%d max=%d got %d ptr=%p count=%d op=%d to=%d\n",
             armci_me,size,(int) MSG_BUFLEN_SMALL,avail,
            _armci_buf_state->buf[avail].buffer, count,operation,to); /* remove warnings (M. Hofmann) */
      fflush(stdout);
    }
