...
void *async_thread_ud_events(void *ctx); /* remove warnings (M. Hofmann) */
  
void init_apm_lock(void);
#if 0 /* remove warnings (M. Hofmann) */
...
static double inittime0=0,inittime1=0,inittime2=0,inittime4=0;
#if 0 /* remove warnings (M. Hofmann) */
static double inittime3=0;
#endif
...
int i;
    if(rr){
       printf("\n%d:print_dscr rr id=%llu sg_lst_len=%d",
              armci_me, (unsigned long long) rr->wr_id, rr->num_sge); /* remove warnings (M. Hofmann) */
       for (i = 0; i < rr->num_sge; i++) {
         printf("\n\t:sg_entry=%d addr=%llx len=%d",
                i, (long long int) rr->sg_list[i].addr, rr->sg_list[i].length); /* remove warnings (M. Hofmann) */
       }
       fflush(stdout);
    }
    if(sr){
       printf("\n%d:print_dscr sr id=%llu opcode=%d sg_lst_len=%d",
              armci_me, (unsigned long long) sr->wr_id, sr->opcode, sr->num_sge); /* remove warnings (M. Hofmann) */
       for (i = 0; i < sr->num_sge; i++) {
         printf("\n\t:sg_entry=%d addr=%llx len=%d",
                i, (long long int) sr->sg_list[i].addr, sr->sg_list[i].length); /* remove warnings (M. Hofmann) */
       }
       fflush(stdout);
    }
}
...
int done=0;
    ...
    if(debug){
       printf("\n%d%s:recv_complete called from %s id=%llu\n",armci_me,
               ((SERVER_CONTEXT)?"(s)":" "),from,(unsigned long long) rcv_dscr->wr_id);fflush(stdout); /* remove warnings (M. Hofmann) */
    }
      ...
      dassert1(1,pdscr->status==IBV_WC_SUCCESS,pdscr->status);
       if(debug){
         if(pdscr->wr_id >= DSCRID_SCATGAT && pdscr->wr_id < DSCRID_SCATGAT_END)
           printf("\n%d:recv from %s complete id=%llu num=%d",armci_me,
             from,(unsigned long long) pdscr->wr_id,rdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].numofrecvs); /* remove warnings (M. Hofmann) */
       }
       if(pdscr->wr_id >= DSCRID_SCATGAT && pdscr->wr_id < DSCRID_SCATGAT_END){
...
/* fflush(stdout); */
    ...
    if(debug) {
       printf("\n%d%s:send_complete called from %s id=%llu nt=%d\n",armci_me,
               ((SERVER_CONTEXT)?"(s)":" "),from,(unsigned long long) snd_dscr->wr_id,numoftimes); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }
...
/*       fflush(stdout); */
       if(SERVER_CONTEXT){
         if(debug)printf("%d:completed id %llu i=%d\n",armci_me,(unsigned long long) pdscr->wr_id,i); /* remove warnings (M. Hofmann) */
         if(pdscr->wr_id >=DSCRID_SCATGAT && pdscr->wr_id < DSCRID_SCATGAT_END){
        ...
        rcq_entries)
{
    ...
    rc = 
#endif
    ibv_query_device(nic->handle, &nic->attr);
    ...
    for (i = 1; i <= 2; i++) {
        ...
        rc = 
#endif
        ibv_query_port(nic->handle, (uint8_t)i, &nic->hca_port);
...
void armci_server_alloc_bufs()
{
    ...
    int mod;
#endif
    int bytes, total, extra =sizeof(struct ibv_recv_wr)*MAX_DESCR+SIXTYFOUR;
...
/* #endif */
   ...
   for (i =0; i<armci_nproc; i++) flag_arr[i] = 9999;
    ...
    if(DEBUG_SERVER){
       printf("%d(s):registered mem %p %dbytes mhandle=%p mharr starts%p\n", /* remove warnings (M. Hofmann) */
...
static void vapi_connect_client()
{
    ...
    if (!armci_use_odcm) {
        ...
        qp_attr_mask = (enum ibv_qp_attr_mask) (IBV_QP_STATE
            | IBV_QP_PKEY_INDEX
            | IBV_QP_PORT
            | IBV_QP_ACCESS_FLAGS); /* remove warnings (M. Hofmann) */
        ...
        qp_attr_mask = (enum ibv_qp_attr_mask) (IBV_QP_STATE
            ...
            | IBV_QP_PATH_MTU
            | IBV_QP_RQ_PSN
            | IBV_QP_MIN_RNR_TIMER); /* remove warnings (M. Hofmann) */
        memset(&qp_attr, 0, sizeof qp_attr);
...
#endif
            qp_attr_mask = (enum ibv_qp_attr_mask) (qp_attr_mask | IBV_QP_AV | IBV_QP_DEST_QPN); /* remove warnings (M. Hofmann) */
#if 0
    ...
    if (!armci_use_odcm) {
        ...
        qp_attr_mask = (enum ibv_qp_attr_mask) (IBV_QP_STATE
            ...
            | IBV_QP_RETRY_CNT
            | IBV_QP_RNR_RETRY
            | IBV_QP_MAX_QP_RD_ATOMIC); /* remove warnings (M. Hofmann) */
                             ...
                             char *buf, int len, armci_vapi_memhndl_t *mhandle)
{
     ...
     sg_entry->addr     = (uint64_t)(uintptr_t)buf; /* remove warnings (M. Hofmann) */
     sg_entry->length   = len;
}
                                  ...
                                  armci_vapi_memhndl_t *rhandle)
{
     ...
     sd->wr.rdma.remote_addr        = (uint64_t)(uintptr_t)rbuf; /* remove warnings (M. Hofmann) */

     if (lhandle) sg_entry->lkey    = lhandle->lkey;
     sg_entry->addr                 = (uint64_t)(uintptr_t)lbuf; /* remove warnings (M. Hofmann) */
     sg_entry->length               = len;
}
...
void armci_server_initial_connection()
{
  int c, rc, i, j;
    ...
    for(c = 0; c < armci_nproc; c++) {
       armci_connect_t *con = CLN_con + c;
       qp_attr_mask = (enum ibv_qp_attr_mask) (qp_attr_mask | IBV_QP_DEST_QPN | IBV_QP_AV); /* remove warnings (M. Hofmann) */
...
/* #endif */
      ...
      dassert1(1,pdscr->status==IBV_WC_SUCCESS,pdscr->status);
       ...
       if (DEBUG_SERVER) {
         ...
         printf("%d(s) : NEW MESSAGE id is %llu \n",armci_me,(unsigned long long) pdscr->wr_id); /* remove warnings (M. Hofmann) */
         fflush(stdout);
       }
#if defined(PEND_BUFS)
...
/* 	 fflush(stdout); */
...
#endif
         if (DEBUG_SERVER) {
           printf("%d(s) : received SCATGAT DATA id = %llu, length = %d\n",
                  armci_me,(unsigned long long) pdscr->wr_id, pdscr->byte_len); /* remove warnings (M. Hofmann) */
           fflush(stdout);
	 }
#if !defined(PEND_BUFS)
	 sr_descr_t *sdscr_arr; /* remove warnings (M. Hofmann) */
	 sdscr_arr = armci_vapi_serv_nbsdscr_array;
	 ...
	 if(sdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].numofsends==0)
	     sdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].tag=0;
#else
	 sr_descr_t *rdscr_arr; /* remove warnings (M. Hofmann) */
	 rdscr_arr = armci_vapi_serv_nbrdscr_array;
	 rdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].numofrecvs--;
	 if(rdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].numofrecvs==0)
	   rdscr_arr[pdscr->wr_id-DSCRID_SCATGAT].tag=0;
       ...
       if (DEBUG_SERVER) {
         printf("%d(s) : request id is %llu operation is %d, length is %d from=%d cbuf->dscr.wr_id=%d\n",
		armci_me,(unsigned long long) pdscr->wr_id,msginfo->operation,pdscr->byte_len,msginfo->from, (int)cbuf->dscr.wr_id); /* remove warnings (M. Hofmann) */
         fflush(stdout);
       }
       ...
       if(DEBUG_SERVER) {
	 printf("%d(s):Came out of poll id=%llu\n",armci_me,(unsigned long long) pdscr->wr_id); /* remove warnings (M. Hofmann) */
	 fflush(stdout);
       }
   ...
   int num;
	 ...
	 if(DEBUG_SERVER) {
	   printf("%d(s) : finished posting scatter\n",armci_me); /* remove warnings (M. Hofmann) */
	   fflush(stdout);
	 }
	 ...
	 if (DEBUG_SERVER) {
            printf("%d(s) : Register_op id is %d, comp_dscr_id is  %llu\n",
                     armci_me,msginfo->operation,(unsigned long long) pdscr->wr_id); /* remove warnings (M. Hofmann) */
            fflush(stdout);
          }
	  ...
	  _armci_pendbuf_post_immbuf(cbufs, msginfo->from);
         ...
         if(DEBUG_SERVER) {
	   printf("%d(s) : request is %llu about to call armci_data_server\n",
		  armci_me, (unsigned long long) pdscr->wr_id); /* remove warnings (M. Hofmann) */
	   fflush(stdout);
         }
#if defined(PEND_BUFS)
...
/*    fflush(stdout);*/
    ...
    if(DEBUG_CLN){
       printf("%d:client sent REQ=%d %d bytes serv=%d qp=%p id =%llu lkey=%d\n",
               armci_me,msginfo->operation,bytes,cluster,
               (SRV_con+cluster)->qp,(unsigned long long) snd_dscr->wr_id,ssg_lst->lkey); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }
    return(0);
}
...
void armci_client_direct_rdma_strided(int operation, int proc,
  ...
  if(DEBUG_CLN) {
    printf("\n%d: in direct rdma strided id=%llu lkey=%u rkey=%u\n",
	   armci_me,(unsigned long long) dirdscr->sdescr.wr_id,(unsigned) lochdl->lkey,(unsigned) remhdl->rkey);fflush(stdout); /* remove warnings (M. Hofmann) */
  }
  ...
  while(armci_stride_info_has_more(&sinfo)) {
    ...
    uint64_t saddr = (uint64_t)(uintptr_t)armci_stride_info_seg_ptr(&sinfo); /* remove warnings (M. Hofmann) */
    uint64_t daddr = (uint64_t)(uintptr_t)armci_stride_info_seg_ptr(&dinfo); /* remove warnings (M. Hofmann) */
    if(operation == PUT) {
...
void armci_server_rdma_strided_to_contig(char *src_ptr, int src_stride_arr[],
  ...
  if(DEBUG_CLN) {
    printf("\n%d: in rdma strided to contig id=%d lkey=%u rkey=%u\n",
	   armci_me,wr_id,(unsigned) loc_memhdl->lkey,(unsigned) rem_memhdl->rkey); /* remove warnings (M. Hofmann) */
    fflush(stdout);
  }
  ...
  numposts = numsegs = 0;
  ctr=0;
  daddr = (uint64_t)(uintptr_t)dst_ptr; /* remove warnings (M. Hofmann) */
  bytes=0;
  while(armci_stride_info_has_more(&sinfo)) {
    sg_entry[ctr].addr = (uint64_t)(uintptr_t)armci_stride_info_seg_ptr(&sinfo); /* remove warnings (M. Hofmann) */
...
void armci_server_rdma_contig_to_strided(char *src_ptr, int proc,
  ...
  numposts = numsegs = 0;
  ctr=0;
  saddr = (uint64_t)(uintptr_t)src_ptr; /* remove warnings (M. Hofmann) */
  bytes=0;
  while(armci_stride_info_has_more(&dinfo)) {
    sg_entry[ctr].addr = (uint64_t)(uintptr_t)armci_stride_info_seg_ptr(&dinfo); /* remove warnings (M. Hofmann) */
    assert(sg_entry[ctr].length == seg_count[0]);
...
char *armci_ReadFromDirectSegment(int proc, request_header_t *msginfo, int len, int *bytes_done) {
  ...
  if(!msginfo->bypass){
...

	{
	  ...
	  int *ptr = sfirst+off;
	  dassert(1,off>=0);
	  dassert(1,(void *)sfirst>(void *)dataptr); /* remove warnings (M. Hofmann) */
	  dassert(1,(void *)ptr>(void *)dataptr); /* remove warnings (M. Hofmann) */
...
struct ibv_recv_wr *bad_wr;
    ...
    if((type==SERV && DEBUG_SERVER) || (type==CLN && DEBUG_CLN)){
       printf("%d(%d) : inside posts scatter dscr, id is %llu\n",
	      armci_me,type,(unsigned long long) scat_dscr->wr_id); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }
    ...
    if((type==SERV && DEBUG_SERVER) || (type==CLN && DEBUG_CLN) ) {
       printf("\n%d: list_length is %d, id is %llu\n",
	      armci_me,scat_dscr->num_sge,(unsigned long long) scat_dscr->wr_id); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }
}
     ...
     int proc, int nbtag, int type, sr_descr_t **srd)
{
    ...
    for(i=0;i<total_of_2D;i++){
       ...
       for(j=0; j<count[1]; j++, vecind++){
         ...
         scat_sglist[y].addr = (uint64_t)(uintptr_t)src1; /* remove warnings (M. Hofmann) */
...
static void posts_gather_desc(sr_descr_t *pend_dscr,int proc,int type)
{
    ...
    if((type==SERV && DEBUG_SERVER) || (type==CLN && DEBUG_CLN)){
       printf("%d: type(client=1)=%d inside posts gather dscr, id is %llu\n",
	      armci_me,type,(unsigned long long) gat_dscr->wr_id); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }
...
/*     gat_dscr->send_flags = 0; */
    ...
    for(i=0;i<total_of_2D;i++){
       ...
       for(j=0; j<count[1]; j++, vecind++){
         ...
         gat_sglist[y].addr = (uint64_t)(uintptr_t)src1; /* remove warnings (M. Hofmann) */
                   ...
                   ARMCI_MEMHDL_T *rhandle)
{
    ...
    printf("--------\n");
    sg->addr= (uint64_t)(uintptr_t)ploc; /* remove warnings (M. Hofmann) */
    if(lhandle)
    sg->lkey = lhandle->lkey;
    sd->sg_list = sg;
    sd->num_sge = 1;
    sd->wr.atomic.remote_addr = (uint64_t)(uintptr_t)prem; /* remove warnings (M. Hofmann) */
    if(rhandle)
       sd->wr.atomic.rkey = rhandle->rkey; /* how do we get the remote key  */
    sd->wr_id = DSCRID_RMW + armci_me;

    if(1){
       printf("%d(c) : finished initialising atomic send desc id is %llu,armci_ime = %d\n",armci_me,(unsigned long long) sd->wr_id,armci_me); /* remove warnings (M. Hofmann) */
       fflush(stdout);
    }   
}
/*\
...
void *async_thread_ud_events(void *context) /* remove warnings (M. Hofmann) */
{
#if 0 /* remove warnings (M. Hofmann) */
    struct ibv_cq *ev_cq;
    void *ev_ctx;
...
int create_cq(void)
{
    ...
    if (armci_me == armci_master) {
        pthread_create(&armci_async_thread[1], NULL,
                async_thread_ud_events, (void *) hca.context); /* remove warnings (M. Hofmann) */
    }
...
int recycle_dead_qp()
{
  return 0; /* remove warnings (M. Hofmann) */
}
...
void process_recv_completion_from_server(armci_ud_rank *h, cbuf *v)
{
    ...
    rc = 
#endif
    ibv_modify_qp(con->qp, &qp_attr, qp_attr_mask);
