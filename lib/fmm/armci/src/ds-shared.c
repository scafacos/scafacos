...
void armci_send_contig_bypass(int proc, request_header_t *msginfo, void *src_ptr, void *rem_ptr, int bytes);
...
void armci_server_rdma_strided_to_contig(char *src_ptr, int src_stride_arr[], int seg_count[], int stride_levels, char *dst_ptr, int proc, request_header_t *msginfo);
...
#else
     int ack=ACK; /* remove warnings (M. Hofmann) */
      ...
      default:
          if(msginfo->format ==VECTOR)
              ...
              if(msginfo->pinned == 1){
                  ...
                  int i,num,id = 0; /* remove warnings (M. Hofmann) */
