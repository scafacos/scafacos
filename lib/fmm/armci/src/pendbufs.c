...
static int att_ops(int attid, int gs, int v) {
  ...
  static int not_first[NUM_ATTRIBUTES]; /*auto-init to zero*/
  static int val[NUM_ATTRIBUTES];
...
/*#warning "PEND_BUFS: Abusing msginfo->tag.ack_ptr for GETS with large descriptors!"*/ /* remove warnings (M. Hofmann) */
      ...
      void *rptr = ((char *)msginfo->tag.ack_ptr) - (uintptr_t)(&((request_header_t *)0)->tag.ack) + IMM_BUF_LEN; /* remove warnings (M. Hofmann) */
...
void armci_pendbuf_service_req(immbuf_t *immbuf) {
  ...
  else if((pbuf = _armci_serv_pendbuf_enqueue(immbuf))) { /* remove warnings (M. Hofmann) */
