...
void armci_acc_1D(int op, void *scale, int proc, void *src, void *dst, int bytes, int lockit)
{
  int rows;
  /* remove warnings (M. Hofmann) */
  typedef void (ATR *func_t)(void*, void*, void*, int*);
  func_t func;
  switch (op){
  case ARMCI_ACC_INT:
    rows = bytes/sizeof(int);
    func = (func_t) I_ACCUMULATE_1D;
    break;
  case ARMCI_ACC_LNG:
    rows = bytes/sizeof(long);
    func = (func_t) L_ACCUMULATE_1D;
    break;
  case ARMCI_ACC_DBL:
    rows = bytes/sizeof(double);
    func = (func_t) D_ACCUMULATE_1D;
    break;
  case ARMCI_ACC_DCP:
    rows = bytes/(2*sizeof(double));
    func = (func_t) Z_ACCUMULATE_1D;
    break;
  case ARMCI_ACC_CPL:
    rows = bytes/(2*sizeof(float));
    func = (func_t) C_ACCUMULATE_1D;
    break;
  case ARMCI_ACC_FLT:
    rows = bytes/sizeof(float);
    func = (func_t) F_ACCUMULATE_1D;
    break;
  default: armci_die("ARMCI accumulate: operation not supported",op);
    func = (func_t) F_ACCUMULATE_1D; /*avoid compiler whining */
  }
		    ...
		    int bytes, int cols, int src_stride, int dst_stride, int lockit)
{
  ...
  /* remove warnings (M. Hofmann) */
  typedef void (ATR *func_t)(void*, int*, int*, void*, int*, void*, int*);
  func_t func;
...
{                  
  ...
  if(stride_levels == 0){
    ...
    switch(stride_levels){
    case 1: 
      DCOPY21(&rows, count+1, ptr, &ld, (void *) buf, &idx); /* remove warnings (M. Hofmann) */
      break;
    case 2: 
#if 0
      for(i=0; i< count[2]; i++){ 
	...
	buf = (char*) ((double*)buf + idx);
      ...
      DCOPY31(&rows, count+1, count+2, ptr, &ld, &ldd, (void *) buf,&idx); /* remove warnings (M. Hofmann) */
	...
	DCOPY21(&rows, count+1,(void *) src, &ld, (void *) buf, &idx); /* remove warnings (M. Hofmann) */
	buf = (char*) ((double*)buf + idx);
      }
    } /*switch */
  } /*else */
...
{                  
  ...
  if(stride_levels == 0){
    ...
    switch(stride_levels){
    case 1: 
      DCOPY12(&rows, count+1, ptr, &ld, (void *) buf, &idx); /* remove warnings (M. Hofmann) */
      break;
    case 2:
#if 0
      for(i=0; i< count[2]; i++){
...
#endif
      ldd = stride_arr[1]/stride_arr[0];   
      DCOPY13(&rows, count+1, count+2, ptr, &ld, &ldd, (void *) buf,&idx); /* remove warnings (M. Hofmann) */
      break;
    default:
      index[2] = 0; unit[2] = 1; total = count[2];
      for(j=3; j<=stride_levels; j++) {
	...
	DCOPY12(&rows, count+1,(void *) src, &ld, (void *) buf, &idx); /* remove warnings (M. Hofmann) */
	buf = (char*) ((double*)buf + idx);
      }
    } /*switch */
  } /*else */
