...
#ifndef DUPLICATE
static int CONCAT(GRIDSORT_PREFIX, CONCAT(GRIDSORT_FRONT_TPROC_BASE, GRIDSORT_FRONT_TPROC_NAME))(GRIDSORT_ELEM_BUF_T *s, GRIDSORT_ELEM_INDEX_T x, void *data)
#else
static void CONCAT(GRIDSORT_PREFIX, CONCAT(GRIDSORT_FRONT_TPROC_BASE, GRIDSORT_FRONT_TPROC_NAME))(GRIDSORT_ELEM_BUF_T *s, GRIDSORT_ELEM_INDEX_T x, void *data, GRIDSORT_INT_T *nprocs, int *procs, GRIDSORT_ELEM_BUF_T *sd)
#endif
{
  void **data_ptrs = data;
...
# ifdef GRIDSORT_DATA1_3
          GRIDSORT_DATA1(sd, nparts)[0] = GRIDSORT_DATA1(s, x)[0];
          GRIDSORT_DATA1(sd, nparts)[1] = GRIDSORT_DATA1(s, x)[1];
          GRIDSORT_DATA1(sd, nparts)[2] = GRIDSORT_DATA1(s, x)[2];
# else
