...
const z_int_t z_crc32_table_size = 256;

const z_crc32_t z_crc32_table[256] = {
...
void z_crc32_make_table(z_crc32_t *tbl) /* z_func z_crc32_make_table */
{
  z_int_t i, j;
  z_crc32_t c;


  for (i = 0; i < z_crc32_table_size; ++i)
  {
    c = i;

    for (j = 0; j < 8; ++j)
    {
      if (c & 1) c = CRC32POLYREV ^ (c >> 1);
      else c >>= 1;
    }
...
#define z_crc32_fmtx  ".8X"
  ...
  printf("z_crc32_t z_crc32_table[%d] = {", (int) z_crc32_table_size);
  for (i = 0; i < z_crc32_table_size; i++)
  {
...
z_crc32_t z_crc32_buffer_update(z_crc32_t crc, const void *buffer, z_int_t length) /* z_func z_crc32_buffer_update */
{
  ...
  for (i = 0; i < length; ++i) crc = z_crc32_table[(crc ^ b[i]) & 0xFF] ^ (crc >> 8);
...
z_crc32_t z_crc32_buffer(const void *buffer, z_int_t length) /* z_func z_crc32_buffer */
{
