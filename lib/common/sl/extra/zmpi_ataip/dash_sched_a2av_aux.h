...
typedef dsint_t ds_aux_acquire_f(struct _ds_sched_a2av_aux_t *aux, dsint_t count, dspint_t proc);
typedef void ds_aux_post_acquire_f(struct _ds_sched_a2av_aux_t *aux);

typedef dsint_t ds_aux_get_count_f(struct _ds_sched_a2av_aux_t *aux, dspint_t proc);
typedef dsint_t ds_aux_get_displ_f(struct _ds_sched_a2av_aux_t *aux, dspint_t proc);

typedef void ds_aux_vacate_f(struct _ds_sched_a2av_aux_t *aux, void *schedptr);

typedef void ds_aux_accept_recv_f(struct _ds_sched_a2av_aux_t *aux, dspint_t proc, dsint_t count);
...
dsint_t ds_aux_acquire(ds_sched_a2av_aux_t *aux, dsint_t count, dspint_t proc);
void ds_aux_post_acquire(ds_sched_a2av_aux_t *aux);

dsint_t ds_aux_get_count(ds_sched_a2av_aux_t *aux, dspint_t proc);
dsint_t ds_aux_get_displ(ds_sched_a2av_aux_t *aux, dspint_t proc);

void ds_aux_vacate(ds_sched_a2av_aux_t *aux, void *schedptr);

void ds_aux_accept_recv(ds_sched_a2av_aux_t *aux, dspint_t proc, dsint_t count);


#ifdef USE_DASH_AUX_STATIC
# include "dash_aux_static.h"
#endif
#ifdef USE_DASH_AUX_HEAP
# include "dash_aux_heap.h"
#endif

#define DS_AUX_TYPE_STATIC  0
#define DS_AUX_TYPE_HEAP    1
