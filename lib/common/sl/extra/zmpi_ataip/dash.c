...
static dsint_t ds_aux_heap_acquire(ds_sched_a2av_aux_t *aux, dsint_t count, dspint_t proc);
static dsint_t ds_aux_heap_get_count(ds_sched_a2av_aux_t *aux, dspint_t proc);
static dsint_t ds_aux_heap_get_displ(ds_sched_a2av_aux_t *aux, dspint_t proc);
static void ds_aux_heap_vacate(ds_sched_a2av_aux_t *aux, void *schedptr);
static void ds_aux_heap_accept_recv(ds_sched_a2av_aux_t *aux, dspint_t proc, dsint_t count);
...
static dsint_t ds_aux_heap_acquire(ds_sched_a2av_aux_t *aux, dsint_t count, dspint_t proc) /* ds_func ds_aux_heap_acquire */
{
  aux_heap_t *d = (aux_heap_t *) aux->data;
  dsint_t i;
  ...
  Z_TRACE_IF(DS_TRACE_IF, "acquired: %" dsint_fmt " (of %" dsint_fmt ") from %" dspint_fmt, d->blocks[i].end - d->blocks[i].begin, count, proc);

  return d->blocks[i].end - d->blocks[i].begin;
}


static dsint_t ds_aux_heap_get_count(ds_sched_a2av_aux_t *aux, dspint_t proc) /* ds_func ds_aux_heap_get_count */
{
  aux_heap_t *d = (aux_heap_t *) aux->data;
  dsint_t i;
...
static dsint_t ds_aux_heap_get_displ(ds_sched_a2av_aux_t *aux, dspint_t proc) /* ds_func ds_aux_heap_get_displ */
{
  aux_heap_t *d = (aux_heap_t *) aux->data;
  dsint_t i;
...
static void ds_aux_heap_vacate(ds_sched_a2av_aux_t *aux, void *schedptr) /* ds_func ds_aux_heap_vacate */
{
  aux_heap_t *d = (aux_heap_t *) aux->data;
  dsint_t i, count;
...
static void ds_aux_heap_accept_recv(ds_sched_a2av_aux_t *aux, dspint_t proc, dsint_t count) /* ds_func ds_aux_heap_accept_recv */
{
  aux_heap_t *d = (aux_heap_t *) aux->data;
  dsint_t i;
...
dsint_t ds_exec_mpi_pre_run(ds_exec_t *exec) /* ds_func ds_exec_mpi_pre_run */
{
...
dsint_t ds_exec_mpi_post_run(ds_exec_t *exec) /* ds_func ds_exec_mpi_post_run */
{
  DEFINE_EXEC_MPI(exec, exec_mpi);
...
dsint_t ds_exec_mpi_make_isendrecv(ds_exec_t *exec) /* ds_func ds_exec_mpi_make_isendrecv */
{
  DEFINE_EXEC_MPI(exec, exec_mpi);
  dsint_t i, req_id;
  ...
  for (i = 0; i < exec->ds->recvs.n; ++i)
  {
    ...
    MPI_Irecv(zmpil_at(dst_addr, exec->ds->recvs.displs[i], &exec_mpi->zmpil_types[dst_exec_id]), exec->ds->recvs.counts[i], exec_mpi->mpi_types[dst_exec_id], exec->ds->recvs.srcs[i], DS_EXEC_MPI_ISENDRECV_TAG, exec->ds->comm, &exec_mpi->reqs[req_id]);
  ...
  for (i = 0; i < exec->ds->sends.n; ++i)
  {
    ...
    MPI_Isend(zmpil_at(src_addr, exec->ds->sends.displs[i], &exec_mpi->zmpil_types[src_exec_id]), exec->ds->sends.counts[i], exec_mpi->mpi_types[src_exec_id], exec->ds->sends.dsts[i], DS_EXEC_MPI_ISENDRECV_TAG, exec->ds->comm, &exec_mpi->reqs[req_id]);
  ...
  for (i = 0; i < exec->ds->recvs.n; ++i)
  {
    ...
    Z_TRACE_IF(DS_TRACE_IF, "received %d from %" dspint_fmt " (expected %" dspint_fmt ")", rcount, exec->ds->recvs.srcs[i], exec->ds->recvs.counts[i]);
...
dsint_t ds_exec_mpi_make_alltoallw(ds_exec_t *exec) /* ds_func ds_exec_mpi_make_alltoallw */
{
  DEFINE_EXEC_MPI(exec, exec_mpi);
  dsint_t i, req_id;
  dsint_t src_buf_id, dst_buf_id;
...
/*      MPI_Type_create_hindexed(1, &blockl, &displ, exec_mpi->mpi_types[dst_exec_id], &exec_mpi->addr_types[exec->ds->sched->bufs[dst_buf_id].addr_id]);*/
    ...
    if (exec_mpi->rcounts[exec->ds->recvs.srcs[i]] == 0)
    {
      exec_mpi->rcounts[exec->ds->recvs.srcs[i]] = exec->ds->recvs.counts[i];
      MPI_Get_address(zmpil_at(dst_addr, exec->ds->recvs.displs[i], &exec_mpi->zmpil_types[dst_exec_id]), &displ);
      exec_mpi->rdispls[exec->ds->recvs.srcs[i]] = displ;
/*      printf("R: %" dsint_fmt " / %p / %" dspint_fmt " / %lld / %d\n", dst_buf_id, dst_addr, exec->ds->recvs.displs[i], (long long) displ, exec_mpi->rdispls[exec->ds->recvs.srcs[i]]);*/
      exec_mpi->rtypes[exec->ds->recvs.srcs[i]] = exec_mpi->mpi_types[dst_exec_id];
  ...
  for (i = 0; i < exec->ds->sends.n; ++i)
  {
    ...
    if (exec_mpi->scounts[exec->ds->sends.dsts[i]] == 0)
    {
      exec_mpi->scounts[exec->ds->sends.dsts[i]] = exec->ds->sends.counts[i];
      MPI_Get_address(zmpil_at(src_addr, exec->ds->sends.displs[i], &exec_mpi->zmpil_types[src_exec_id]), &displ);
      exec_mpi->sdispls[exec->ds->sends.dsts[i]] = displ;
/*      printf("S: %" dsint_fmt " / %p / %" dspint_fmt " / %lld / %d\n", src_buf_id, src_addr, exec->ds->sends.displs[i], (long long) displ, exec_mpi->sdispls[exec->ds->sends.dsts[i]]);*/
      exec_mpi->stypes[exec->ds->sends.dsts[i]] = exec_mpi->mpi_types[src_exec_id];
  ...
  for (i = 0; i < exec->ds->recvs.n; ++i)
  {
    if (exec_mpi->rcounts[exec->ds->recvs.srcs[i]] > 0)
    {
      rcount = exec_mpi->rcounts[exec->ds->recvs.srcs[i]];
      exec_mpi->rcounts[exec->ds->recvs.srcs[i]] = 0;
...
void ds_exec_mpi_move(ds_exec_t *exec, dsint_t exec_id, dsint_t src_buf_id, dsint_t src_displs, dsint_t dst_buf_id, dsint_t dst_displs, dsint_t count) /* ds_func ds_exec_mpi_move */
{
  DEFINE_EXEC_MPI(exec, exec_mpi);
  void *src_addr, *dst_addr;


  src_addr = zmpil_at(exec->addrs[exec->ds->sched->bufs[src_buf_id].addr_id], src_displs, &exec_mpi->zmpil_types[exec_id]);
  dst_addr = zmpil_at(exec->addrs[exec->ds->sched->bufs[dst_buf_id].addr_id], dst_displs, &exec_mpi->zmpil_types[exec_id]);
...
void ds_exec_mpi_sendrecv_replace(ds_exec_t *exec, int proc, dsint_t exec_id, dsint_t buf_id, dspint_t displ, dspint_t count) /* ds_func ds_exec_mpi_sendrecv_replace */
{
  DEFINE_EXEC_MPI(exec, exec_mpi);
  ...
  Z_TRACE_IF(DS_TRACE_IF, "buf_id: %" dsint_fmt ", displ: %" dspint_fmt ", count: %" dspint_fmt, buf_id, displ, count);
...
dsint_t ds_exec_mpi_add_type(ds_exec_t *exec, MPI_Datatype type) /* ds_func ds_exec_mpi_add_type */
{
  ...
  if (exec_mpi->ntypes < DS_MAX_NBUFFERS)
  {
