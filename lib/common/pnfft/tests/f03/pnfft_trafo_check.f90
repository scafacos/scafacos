...
contains
  ...
  ! Initialize nonequispaced nodes
    call init_random_x(lower_border, upper_border, local_M, &
       x)
  write(*,*) "local_M = ", local_M, "lower_border = ", lower_border
  write(*,*) "upper_border = ", upper_border
  write(*,*) x(1:3,1)
  write(*,*) x(1:3,4)
  ...
  ! Execute parallel NFFT
    ...
    do k1=1,local_N(1)
      do k2=1,local_N(2)
        do k3=1,local_N(3)
          local_f_hat_sum = local_f_hat_sum + cdabs(f_hat(k3,k2,k1))
        enddo
      enddo
    enddo
...
program main
  ...
  complex(C_DOUBLE_COMPLEX), pointer :: f1(:)=>null(), f2(:)
  type(C_PTR) :: cf1, cf2
