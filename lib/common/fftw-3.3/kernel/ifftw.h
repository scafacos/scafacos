...
#define CONCAT(prefix, name) prefix ## name
  ...
  typedef float R;
# define X(name) CONCAT(fcs_fftwf_, name)
#elif defined(FFTW_LDOUBLE)
  typedef long double R;
# define X(name) CONCAT(fcs_fftwl_, name)
# define TRIGREAL_IS_LONG_DOUBLE
#elif defined(FFTW_QUAD)
  typedef __float128 R;
# define X(name) CONCAT(fcs_fftwq_, name)
# define TRIGREAL_IS_QUAD
#else
  typedef double R;
# define X(name) CONCAT(fcs_fftw_, name)
#endif
...
typedef INT stride;
#define WS(stride, i)  (stride * i)
#define fcs_fftwf_mkstride(n, stride) stride
#define fcs_fftw_mkstride(n, stride) stride
#define fcs_fftwl_mkstride(n, stride) stride
#define fcs_fftwf_stride_destroy(p) ((void) p)
#define fcs_fftw_stride_destroy(p) ((void) p)
#define fcs_fftwl_stride_destroy(p) ((void) p)
