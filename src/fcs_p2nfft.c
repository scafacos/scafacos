...
FCSResult fcs_p2nfft_set_parameter(FCS handle, fcs_bool continue_on_errors, char **current, char **next, fcs_int *matched)
{
  ...
  *matched = 0;
  ...
  FCS_PARSE_IF_PARAM_THEN_FUNC1_GOTO_NEXT("p2nfft_kernel",           p2nfft_set_kernel,                    FCS_PARSE_VAL(fcs_int));
  FCS_PARSE_IF_PARAM_THEN_FUNC1_GOTO_NEXT("p2nfft_kernel_name",      p2nfft_set_kernel_by_name,            FCS_PARSE_VAL(fcs_p_char_t));
