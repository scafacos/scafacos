...
for { set i 0 } { $i <= [setmd max_part] } { incr i } {
    ...
    if { [has_feature "KEEP_KPART"] } {
	set f_ks [part $i print force_k]
	set f_rs [list \
		      [expr [lindex $resF 0] - [lindex $f_ks 0]] \
		      [expr [lindex $resF 1] - [lindex $f_ks 1]] \
		      [expr [lindex $resF 2] - [lindex $f_ks 2]] \
		     ]
    }
    
    # puts "pid=$i"
    # puts "       force=[lindex $resF 0] [lindex $resF 1] [lindex $resF 2]"
    # puts "  near_force=[lindex $f_ks 0] [lindex $f_ks 1] [lindex $f_ks 2]"
    # puts "   far_force=[lindex $f_rs 0] [lindex $f_rs 1] [lindex $f_rs 2]"
}
