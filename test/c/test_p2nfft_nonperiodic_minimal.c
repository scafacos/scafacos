...
int main(int argc, char **argv)
{
  ...
  pid = num_particles = 0;
  for (px = 0; px < TEST_N_PARTICLES; ++px, ++pid) {
    if (pid % comm_size == comm_rank) {
/* debug near field computation */
positions[3*num_particles] = TEST_BOX_SIZE*(px % 2); //px + 0.5;
//      positions[3*num_particles] = 1.0-(px % 2)*0.1; //px + 0.5;
      positions[3*num_particles + 1] = TEST_BOX_SIZE/2.0;
      positions[3*num_particles + 2] = TEST_BOX_SIZE/2.0;
      charges[num_particles] = 1.0-(px % 2)*2;
      ++num_particles;
    }
  }
...
//  fcs_int periodicity[] = {1, 1, 1};
  ...
  if(!comm_rank)
    for(pid=0; pid<num_particles; ++pid)
      printf("x = [%" FCS_LMOD_FLOAT "e %" FCS_LMOD_FLOAT "e %" FCS_LMOD_FLOAT "e]\n", positions[3*pid+0], positions[3*pid+1], positions[3*pid+2]);
  ...
  MPI_Reduce(&direct_energy, &direct_total_energy, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&p2nfft_energy, &p2nfft_total_energy, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
  MPI_Reduce(&sqr_sum, &total_sqr_sum, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
